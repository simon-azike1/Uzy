...
import {
  ...
  Plus, MoreVertical, Trash2, Star, LogOut,
...
const STATUS_CONFIG = {
  applied:        { label: 'Applied',      color: '#2D4263' },
  'under-review': { label: 'Under Review', color: '#F59E0B' },
  accepted:       { label: 'Accepted',     color: '#10B981' },
  rejected:       { label: 'Rejected',     color: '#C84B31' },
  'no-response':  { label: 'No Response',  color: '#9CA3AF' },
}

const getStatusBadgeStyle = (status, dark) => {
  const styles = {
    applied:        { bg: dark ? '#2D426330' : '#2D426315', text: dark ? '#ECDBBA' : '#2D4263',   border: '#2D426344' },
    'under-review': { bg: dark ? '#F59E0B15' : '#F59E0B10', text: dark ? '#FCD34D' : '#D97706',   border: '#F59E0B33' },
    accepted:       { bg: dark ? '#10B98115' : '#10B98110', text: dark ? '#6EE7B7' : '#059669',   border: '#10B98133' },
    rejected:       { bg: dark ? '#C84B3115' : '#C84B3110', text: dark ? '#C84B31' : '#C84B31',   border: '#C84B3133' },
    'no-response':  { bg: dark ? '#9CA3AF10' : '#9CA3AF08', text: dark ? '#9CA3AF' : '#6B7280',   border: '#9CA3AF22' },
  }
...
const exportCSV = (T) => {
  const user = JSON.parse(localStorage.getItem('uzy_user') || '{}')
  const token = user?.token
  fetch(`${API_BASE}/applications/export/csv`, { headers: { Authorization: `Bearer ${token}` } })
...
const buildMonthlyData = (apps) => { const now = new Date(); return Array.from({ length: 6 }, (_, i) => { const d = new Date(now.getFullYear(), now.getMonth() - (5 - i), 1); return { name: MONTHS[d.getMonth()], count: apps.filter(a => { const ad = new Date(a.appliedDate); return ad.getMonth() === d.getMonth() && ad.getFullYear() === d.getFullYear() }).length } }) }
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return <div className="rounded-xl border px-3 py-2 text-xs shadow-xl" style={{ background: T.surfaceHigh, borderColor: T.border, color: T.text }}><p className="font-bold mb-1">{label}</p>{payload.map((p,i)=><p key={i} style={{color:p.color||T.accent}}>{p.name}: {p.value}</p>)}</div>
}
...
const ThemeToggle = () => {
  ...
  return (
    <button onClick={toggleTheme}
      ...
      <motion.div key={dark ? 'moon' : 'sun'} initial={{ rotate: -30, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} transition={{ duration: 0.2 }}>
...
const SettingsTab = ({ onProfileUpdate }) => {
  ...
  useEffect(() => {
    profileApi.get().then(res => {
      if (res.success) {
        ...
        setPrefs({ targetRole: res.data.preferences?.targetRole || '', industry: res.data.preferences?.industry || '', salaryMin: res.data.preferences?.salaryMin || '', salaryMax: res.data.preferences?.salaryMax || '', jobType: res.data.preferences?.jobType || 'full-time' })
      }
      setLoading(false)
    }).catch(() => setLoading(false))
  }, [])

  const handleSaveProfile = async () => { setSaving(true); const res = await profileApi.update({ name: form.name, email: form.email, avatarColor: form.avatarColor }); if (res.success) { showToast('Profile updated!'); onProfileUpdate(res.data) } else showToast(res.message || 'Failed', 'error'); setSaving(false) }
  const handleSavePrefs = async () => { setSaving(true); const res = await profileApi.update({ preferences: { ...prefs, salaryMin: Number(prefs.salaryMin)||null, salaryMax: Number(prefs.salaryMax)||null } }); if (res.success) showToast('Preferences saved!'); else showToast(res.message||'Failed','error'); setSaving(false) }
  const handleChangePassword = async () => { if (!pw.currentPassword||!pw.newPassword) return showToast('Both fields required','error'); setPwSaving(true); const res = await profileApi.changePassword(pw); if (res.success) { showToast('Password changed!'); setPw({ currentPassword:'', newPassword:'' }) } else showToast(res.message||'Failed','error'); setPwSaving(false) }
  ...
  if (loading) return <div className="py-24 text-center"><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }} className="w-6 h-6 rounded-full mx-auto" style={{ border: '2px solid ' + T.accent, borderTopColor: 'transparent' }} /></div>

  const inputStyle = { background: T.bg, border: '1px solid ' + T.border, color: T.text }
  const cardStyle  = { background: T.surface, borderColor: T.border }

  return (
    <>
      <AnimatePresence>
        {toast && <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
          ...
          <AnimatePresence mode="wait">
            {activeSection === 'profile' && (
              <motion.div key="profile" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                className="rounded-2xl border p-6" style={cardStyle}>
                <h3 className="font-black text-base mb-1" style={{ color: T.text }}>Profile Information</h3>
                <p className="text-xs mb-6" style={{ color: T.textFaint }}>Update your name and email address</p>
                <div className="space-y-4">
                  {[['Full Name','name','text'],['Email Address','email','email']].map(([label,key,type]) => (
                    <div key={key}>
                      <label className="text-xs font-medium mb-1.5 block" style={{ color: T.textMuted }}>{label}</label>
                      <input type={type} value={form[key]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))}
                        className="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none" style={inputStyle} />
                    </div>
                  ))}
                </div>
                <button onClick={handleSaveProfile} disabled={saving} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50" style={{ background: T.accent, color: '#fff' }}>
            ...
            {activeSection === 'preferences' && (
              <motion.div key="prefs" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                ...
                <p className="text-xs mb-6" style={{ color: T.textFaint }}>Help us tailor your experience</p>
                <div className="space-y-4">
                  {[['Target Role','targetRole','e.g. Frontend Engineer'],['Industry','industry','e.g. Technology']].map(([label,key,placeholder]) => (
                    <div key={key}>
                      <label className="text-xs font-medium mb-1.5 block" style={{ color: T.textMuted }}>{label}</label>
                      <input value={prefs[key]} onChange={e => setPrefs(p => ({ ...p, [key]: e.target.value }))} placeholder={placeholder}
                        className="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none" style={inputStyle} />
                    </div>
                  ))}
                  <div>
                    <label className="text-xs font-medium mb-1.5 block" style={{ color: T.textMuted }}>Job Type</label>
                    <select value={prefs.jobType} onChange={e => setPrefs(p => ({ ...p, jobType: e.target.value }))}
                      className="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none" style={inputStyle}>
                      {[['full-time','Full Time'],['part-time','Part Time'],['contract','Contract'],['internship','Internship'],['remote','Remote']].map(([v,l]) => <option key={v} value={v}>{l}</option>)}
                    </select>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    {[['Min Salary ($)','salaryMin'],['Max Salary ($)','salaryMax']].map(([label,key]) => (
                      <div key={key}>
                        <label className="text-xs font-medium mb-1.5 block" style={{ color: T.textMuted }}>{label}</label>
                        <input type="number" value={prefs[key]} onChange={e => setPrefs(p => ({ ...p, [key]: e.target.value }))} placeholder="0"
                          className="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none" style={inputStyle} />
                      </div>
                    ))}
                  </div>
                </div>
                <button onClick={handleSavePrefs} disabled={saving} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50" style={{ background: T.accent, color: '#fff' }}>
            ...
            {activeSection === 'security' && (
              <motion.div key="security" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                ...
                <p className="text-xs mb-6" style={{ color: T.textFaint }}>Choose a strong password of at least 6 characters</p>
                <div className="space-y-4">
                  {[['Current Password','currentPassword','current'],['New Password','newPassword','new']].map(([label,key,vis]) => (
                    <div key={key}>
                      <label className="text-xs font-medium mb-1.5 block" style={{ color: T.textMuted }}>{label}</label>
                      <div className="relative">
                        <input type={showPw[vis] ? 'text' : 'password'} value={pw[key]} onChange={e => setPw(p => ({ ...p, [key]: e.target.value }))}
                          className="w-full rounded-xl px-4 py-2.5 pr-10 text-sm focus:outline-none" style={inputStyle} />
                        <button onClick={() => setShowPw(p => ({ ...p, [vis]: !p[vis] }))} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: T.textFaint }}>
                ...
                <button onClick={handleChangePassword} disabled={pwSaving} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50" style={{ background: T.accent, color: '#fff' }}>
            ...
            {activeSection === 'appearance' && (
              <motion.div key="appearance" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                className="rounded-2xl border p-6" style={cardStyle}>
                <h3 className="font-black text-base mb-1" style={{ color: T.text }}>Theme</h3>
                <p className="text-xs mb-6" style={{ color: T.textFaint }}>Choose between dark and light mode</p>
                <div className="flex gap-4 mb-8">
                  {[['dark','Dark','#191919','#ECDBBA'],['light','Light','#F5F5F5','#1A1A1A']].map(([id,label,bg,text]) => (
                    <button key={id} onClick={() => { if ((id==='dark')!==dark) toggleTheme() }}
                      className="flex-1 rounded-2xl p-4 border-2 transition-all"
                      style={{ background: bg, borderColor: (id==='dark')===dark ? T.accent : T.border }}>
                      <div className="w-8 h-8 rounded-lg mb-3 mx-auto" style={{ background: id==='dark'?'#2D4263':'#E5E5E5' }} />
                      <p className="text-xs font-bold text-center" style={{ color: text }}>{label}</p>
                      {(id==='dark')===dark && <p className="text-[10px] text-center mt-1" style={{ color: T.accent }}>Active</p>}
                    </button>
                  ))}
                </div>
                <h3 className="font-black text-base mb-1" style={{ color: T.text }}>Avatar Color</h3>
                <p className="text-xs mb-4" style={{ color: T.textFaint }}>Pick a color for your profile avatar</p>
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-black" style={{ background: form.avatarColor, color: '#fff' }}>
                    {form.name?.[0]?.toUpperCase() || 'U'}
                  </div>
                  <div className="flex flex-wrap gap-3">
                    {AVATAR_COLORS.map(color => (
                      <button key={color} onClick={() => setForm(p => ({ ...p, avatarColor: color }))}
                        ...
                        style={{ background: color, outline: form.avatarColor === color ? '2px solid ' + T.text : '2px solid transparent', outlineOffset: '2px', transform: form.avatarColor === color ? 'scale(1.1)' : 'scale(1)' }} />
                    ))}
                  </div>
                </div>
                <button onClick={handleSaveProfile} disabled={saving} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50" style={{ background: T.accent, color: '#fff' }}>
...
const AnalyticsTab = ({ applications, stats }) => {
  ...
  if (!applications.length) return (
    <div className="py-24 text-center">
      <BarChart2 size={36} className="mx-auto mb-4" style={{ color: T.textFaint }} />
      <p className="text-base font-bold mb-2" style={{ color: T.textMuted }}>No data yet</p>
      <p className="text-sm" style={{ color: T.textFaint }}>Add applications to see your analytics</p>
    </div>
  )

  const card = { background: T.surface, borderColor: T.border }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((kpi, i) => (
          <motion.div key={kpi.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            className="rounded-2xl p-5 border" style={card}>
            ...
            <p className="text-xs font-medium uppercase tracking-wider mb-1" style={{ color: T.textMuted }}>{kpi.label}</p>
      ...
      <div className="grid md:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="md:col-span-2 rounded-2xl border p-6" style={card}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={monthlyData}>
              <defs><linearGradient id="areaGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#C84B31" stopOpacity={0.3} /><stop offset="100%" stopColor="#C84B31" stopOpacity={0} /></linearGradient></defs>
              ...
              <Area type="monotone" dataKey="count" name="Applications" stroke="#C84B31" strokeWidth={2} fill="url(#areaGrad)" dot={{ fill: '#C84B31', r: 4, strokeWidth: 0 }} activeDot={{ r: 6, fill: '#C84B31' }} />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
          className="rounded-2xl border p-6" style={card}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <PieChart><Pie data={statusData} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="value">{statusData.map((entry,i)=><Cell key={i} fill={entry.color} stroke="transparent"/>)}</Pie><Tooltip content={<CustomTooltip/>}/></PieChart>
          </ResponsiveContainer>
          <div className="space-y-2 mt-2">
            {statusData.map(s => (
              <div key={s.key} className="flex items-center justify-between">
                <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{ background: s.color }} /><span className="text-xs" style={{ color: T.textMuted }}>{s.name}</span></div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-2xl border p-6" style={card}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={companyData} layout="vertical">
              ...
              <XAxis type="number" tick={{ fill: T.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} />
              <YAxis type="category" dataKey="name" width={80} tick={{ fill: T.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="count" name="Applications" radius={[0,6,6,0]}>{companyData.map((_,i)=><Cell key={i} fill={i===0?'#C84B31':'#2D4263'}/>)}</Bar>
            </BarChart>
          </ResponsiveContainer>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="rounded-2xl border p-6" style={card}>
          ...
          <div className="space-y-1.5">
            {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((day, di) => (
              <div key={day} className="flex items-center gap-1.5">
                <span className="text-[10px] w-6 shrink-0" style={{ color: T.textFaint }}>{day}</span>
                <div className="flex gap-1.5 flex-1">
                  {grid.map((week, wi) => <div key={wi} title={week[di]?.date+': '+week[di]?.count+' apps'} className="flex-1 rounded aspect-square" style={{ background: heatColor(week[di]?.count), minWidth: '12px', maxWidth: '20px' }} />)}
                </div>
              </div>
            ))}
          </div>
          <div className="flex items-center gap-2 mt-4">
            ...
            {[0,0.25,0.5,0.75,1].map((v,i)=><div key={i} className="w-3 h-3 rounded-sm" style={{ background: v===0?T.border:'#C84B31'+Math.round(v*255).toString(16).padStart(2,'0') }}/>)}
            <span className="text-[10px]" style={{ color: T.textFaint }}>More</span>
          </div>
        </motion.div>
      </div>

      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-2xl border p-6" style={card}>
        <div className="flex items-center justify-between mb-6">
          <div><p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: T.accent }}>Performance</p><h3 className="text-base font-black" style={{ color: T.text }}>Response Metrics</h3></div>
          <span className="text-3xl font-black" style={{ color: T.accent }}>{responseRate}%</span>
        </div>
        <div className="space-y-4">
          {[{ label:'Response Rate', value:responseRate, color:'#10B981' },{ label:'Rejection Rate', value:stats.total?Math.round((stats.rejected||0)/stats.total*100):0, color:'#C84B31' },{ label:'Under Review', value:stats.total?Math.round((stats.underReview||0)/stats.total*100):0, color:'#F59E0B' },{ label:'Acceptance Rate', value:stats.total?Math.round((stats.accepted||0)/stats.total*100):0, color:'#2D4263' }].map(row=>(
            <div key={row.label}>
              <div className="flex items-center justify-between mb-1.5"><span className="text-xs font-medium" style={{ color: T.textMuted }}>{row.label}</span><span className="text-xs font-bold" style={{ color: T.text }}>{row.value}%</span></div>
              <div className="h-1.5 rounded-full overflow-hidden" style={{ background: T.border }}>
                <motion.div initial={{ width: 0 }} animate={{ width: row.value + '%' }} transition={{ duration: 0.8, delay: 0.4, ease: [0.23,1,0.32,1] }} className="h-full rounded-full" style={{ background: row.color }} />
              </div>
            </div>
          ))}
        </div>
...
const EmailConnectBanner = ({ onConnected }) => {
  ...
  const handleConnect = async () => { setConn(true); try { const d=await emailApi.connectGoogle(); if(d.url) window.location.href=d.url } catch { showToast('Failed','error'); setConn(false) } }
  ...
  return (
    <>
      <AnimatePresence>
        {toast && <motion.div initial={{opacity:0,y:-16}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-16}} className="fixed top-5 right-5 z-50 px-4 py-3 rounded-xl text-sm font-medium shadow-xl border"
          style={{background:toast.type==='error'?'#C84B3120':T.primary+'20',borderColor:toast.type==='error'?'#C84B3144':T.primary+'44',color:T.text}}>{toast.msg}</motion.div>}
      </AnimatePresence>
      <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.25,duration:0.4}}
        ...
        style={{background:isConnected?T.primary+'12':'#C84B3108',borderColor:isConnected?T.primary+'44':'#C84B3122'}}>
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{background:isConnected?T.primary+'33':'#C84B3120'}}>
            <Mail size={18} style={{color:isConnected?T.text:T.accent}}/>
          </div>
          <div>
            {isConnected ? (
              <><div className="flex items-center gap-2 mb-0.5"><p className="text-sm font-bold" style={{color:T.text}}>Gmail Connected</p><span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full" style={{background:'#25D36620',color:'#25D366'}}><CheckCheck size={9}/> Active</span></div><p className="text-xs" style={{color:T.textFaint}}>{status?.google?.email} — auto-detecting job applications</p></>
            ) : (
              <><p className="text-sm font-bold mb-0.5" style={{color:T.text}}>Connect Gmail to auto-detect applications</p><p className="text-xs" style={{color:T.textFaint}}>We scan your inbox for job confirmation emails and add them automatically</p></>
            )}
          </div>
        </div>
        <div className="shrink-0">
          {isConnected
            ? <button onClick={handleDisconnect} disabled={disconnecting} className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold disabled:opacity-50" style={{background:'#C84B3115',color:T.accent,border:'1px solid #C84B3133'}}>{disconnecting?<><Loader2 size={13} className="animate-spin"/>Disconnecting...</>:<><Unlink size={13}/>Disconnect</>}</button>
            : <button onClick={handleConnect} disabled={connecting} className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold disabled:opacity-50" style={{background:T.accent,color:'#fff'}}>{connecting?<><Loader2 size={13} className="animate-spin"/>Connecting...</>:<><Link size={13}/>Connect Gmail</>}</button>
          }
        </div>
      </motion.div>
    </>
...
const StatusBadge = ({ status }) => {
  ...
  const s = getStatusBadgeStyle(status, dark)
  return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border" style={{ background: s.bg, color: s.text, borderColor: s.border }}>
...
const StatCard = ({ icon: Icon, label, value, accent, delay }) => {
  ...
  return (
    <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay,duration:0.4,ease:[0.23,1,0.32,1]}}
      className="rounded-2xl p-5 border transition-colors" style={{background:T.surface,borderColor:T.border}}>
      <div className="inline-flex p-2 rounded-xl mb-3" style={{background:accent?'#C84B3122':T.primary+'22'}}>
        <Icon size={16} style={{color:accent?T.accent:T.primary}}/>
      </div>
      <p className="text-xs font-medium uppercase tracking-wider mb-1" style={{color:T.textMuted}}>{label}</p>
...
const AddModal = ({ onClose, onAdd }) => {
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
      <motion.div className="absolute inset-0 backdrop-blur-sm" style={{background:T.bg+'cc'}} onClick={onClose}/>
      <motion.div className="relative w-full max-w-lg rounded-2xl p-6 shadow-2xl border" style={{background:T.surface,borderColor:T.primary}}
        ...
        <div className="space-y-4">
          {[['Company *','company'],['Job Role *','role'],['Industry','industry'],['Location','location'],['Job URL','jobUrl']].map(([label,key])=>(
            <div key={key}>
              <label className="text-xs font-medium mb-1.5 block" style={{color:T.textMuted}}>{label}</label>
              <input value={form[key]} onChange={e=>set(key,e.target.value)} className="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none" style={inputStyle}/>
            </div>
          ))}
          <div className="grid grid-cols-2 gap-4">
            {[['Status','status',Object.entries(STATUS_CONFIG).map(([v,c])=>[v,c.label])],['Priority','priority',[['high','High'],['medium','Medium'],['low','Low']]]].map(([label,key,opts])=>(
              <div key={key}>
                <label className="text-xs font-medium mb-1.5 block" style={{color:T.textMuted}}>{label}</label>
                <select value={form[key]} onChange={e=>set(key,e.target.value)} className="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none" style={inputStyle}>
          ...
          <div>
            <label className="text-xs font-medium mb-1.5 block" style={{color:T.textMuted}}>Notes</label>
            <textarea rows={3} value={form.notes} onChange={e=>set('notes',e.target.value)} className="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none resize-none" style={inputStyle}/>
          </div>
          {error&&<p className="text-sm" style={{color:T.accent}}>{error}</p>}
        </div>
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{border:'1px solid '+T.primary,color:T.textMuted}}>Cancel</button>
          <button onClick={handleSubmit} disabled={loading} className="flex-1 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50" style={{background:T.accent,color:'#fff'}}>
...
export default function Dashboard() {
  ...
  const handleLogout = () => { logout(); window.location.href = '/home' }
    ...
    <div className="min-h-screen transition-colors duration-300" style={{background:T.bg,color:T.text}}>
      ...
      <aside className="fixed left-0 top-0 h-full w-16 flex flex-col items-center py-5 gap-4 z-20 border-r transition-colors duration-300"
        ...
        <nav className="flex flex-col gap-2 flex-1">
          {TABS.map(({id,icon:Icon,label})=>(
            <button key={id} title={label} onClick={()=>setActiveTab(id)}
              ...
              style={{background:activeTab===id?T.primary+'44':'transparent',color:activeTab===id?T.text:T.textFaint,border:activeTab===id?'1px solid '+T.primary+'66':'1px solid transparent'}}>
              <Icon size={18}/>
            </button>
          ))}
        </nav>
        ...
        <button onClick={toggleTheme} title={dark?'Light mode':'Dark mode'}
          ...
          <motion.div key={dark?'sun':'moon'} initial={{rotate:-30,opacity:0}} animate={{rotate:0,opacity:1}} transition={{duration:0.2}}>
            {dark ? <Sun size={18}/> : <Moon size={18}/>}
          </motion.div>
        </button>
        <button onClick={handleLogout} title="Logout" className="w-10 h-10 rounded-xl flex items-center justify-center" style={{color:T.textFaint}}>
          <LogOut size={18}/>
        </button>
      </aside>
      ...
      <main className="ml-16 p-8">
        <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} transition={{duration:0.35}}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5 rounded-xl px-3 py-2 border" style={{background:T.surface,borderColor:T.border}}>
              <div className="w-6 h-6 rounded-lg flex items-center justify-center font-black text-xs" style={{background:avatarColor,color:'#fff'}}>
                {user?.name?.[0]?.toUpperCase()||'U'}
              </div>
                ...
                <div className="overflow-x-auto">
                  ...
                  {filtered.length===0&&!loading&&(
                    <div className="py-16 text-center">
                      ...
                      <p className="text-sm" style={{color:T.textFaint}}>No applications found</p>
              ...
              <div className="grid grid-cols-3 gap-4">
                {[{step:'01',title:'Connect',desc:'Authorize Gmail access via Google OAuth'},{step:'02',title:'Scan',desc:'We scan the past 3 months of emails'},{step:'03',title:'Done',desc:'Applications appear in your dashboard'}].map(s=>(
                  <div key={s.step} className="rounded-xl p-4 border" style={{background:T.surfaceHigh,borderColor:T.border}}>
                    ...
                    <p className="text-xs" style={{color:T.textFaint}}>{s.desc}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab==='settings' && <SettingsTab onProfileUpdate={(data)=>setAvatarColor(data.avatarColor||'#C84B31')}/>}
      </main>
